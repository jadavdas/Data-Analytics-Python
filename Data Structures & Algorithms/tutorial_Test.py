...
"""
#Solution:
values=input("Enter comman-separated numbers: ")
l=values.split(",")
t=tuple(l)
print(l)
print(t)
...
h=30
value = []
items=[x for x in input("Enter comma separated input: ").split(',')]
for d in items:
...
"""
#Solution:
input_str = input("Enter comma separated 2 digit input: ")
...
#Solution:
value = []
items=[x for x in input("Enter comma separated 4 digit binary: ").split(',')]
for p in items:
    intp = int(p, 2)
    if not intp%5:
        value.append(p)
...
"""
#Solution:
s = input("Enter raw sentence with letter and number: ")
...
Question:
Use a list comprehension to search each odd number in a list. The list is input by a sequence of comma-separated numbers.
Suppose the following input is supplied to the program:
1,2,3,4,5,6,7,8,9
Then, the output should be:
1,3,5,7,9
...
netAmount = 0
while True:
    s = input("Enter value as D/W amount: ")
    if not s:
        break
    values = s.split(" ")
    operation = values[0]
...
pos = [0,0]
while True:
    s = input("Enter the movement in this format (U/D  number or L/R number): ")
    if not s:
        break
    movement = s.split(" ")
